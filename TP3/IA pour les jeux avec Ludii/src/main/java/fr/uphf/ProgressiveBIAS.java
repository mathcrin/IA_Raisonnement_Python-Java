...
public class ProgressiveBIAS extends AI
{

    /*TODO: pour l'algorithme UCB, au coefficient d'exploration est ajouté un biais basé sur une heuristique de jeu.
     *  Pour cela il faut se baser sur des connaissance du jeu, par exemple pour le jeu d'othello on donne plus de point au coup qui permet de prendre un coin.
     */
    //-------------------------------------------------------------------------
    ...
    public static Node select(final Node current)
    {
        ...
        for (int i = 0; i < numChildren; ++i)
        {
            ...
            // Ajout du biais basé sur l'heuristique du jeu
            final double bias = cornerBias(child.moveFromParent);

            final double ucb1Value = exploit + explore + bias;
    ...
    /**
     * Méthode pour calculer le biais basé sur l'heuristique du jeu.
     * Dans cet exemple, on donne plus de points aux coups qui permettent de prendre un coin.
     */
    private static double cornerBias(final Move move)
    {
        // Vérifiez si le coup permet de prendre un coin
        // Vous devrez implémenter cette logique en fonction de votre jeu
        if (isCornerMove(move))
        {
            // Si le coup permet de prendre un coin, ajoutez un biais
            return 1.0;
        }
        else
        {
            // Sinon, ne pas ajouter de biais
            return 0.0;
        }
    }

    /**
     * Méthode pour vérifier si un coup permet de prendre un coin.
     * Vous devrez implémenter cette logique en fonction de votre jeu.
     */
    private static boolean isCornerMove(final Move move)
    {
        // Implémentez la logique pour vérifier si le coup est un coup de coin
        return false;
    }
